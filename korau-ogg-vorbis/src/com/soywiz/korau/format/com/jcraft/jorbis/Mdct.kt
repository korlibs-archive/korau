...
internal class Mdct {
    ...
    fun init(n: Int) {
        ...
        log2n = Math.rint(Math.log(n.toDouble()) / Math.log(2.0)).toInt()
        this.n = n
        ...
        // trig lookups...
        for (i in 0..n / 4 - 1) {
            trig[AE + i * 2] = Math.cos(Math.PI / n * (4 * i)).toFloat()
            trig[AO + i * 2] = (-Math.sin(Math.PI / n * (4 * i))).toFloat()
            trig[BE + i * 2] = Math.cos(Math.PI / (2 * n) * (2 * i + 1)).toFloat()
            trig[BO + i * 2] = Math.sin(Math.PI / (2 * n) * (2 * i + 1)).toFloat()
        }
        for (i in 0..n / 8 - 1) {
            trig[CE + i * 2] = Math.cos(Math.PI / n * (4 * i + 2)).toFloat()
            trig[CO + i * 2] = (-Math.sin(Math.PI / n * (4 * i + 2))).toFloat()
        }
    ...
    @Synchronized fun backward(`in`: FloatArray, out: FloatArray) {
        if (_x.size < n / 2) {
            _x = FloatArray(n / 2)
        }
        if (_w.size < n / 2) {
