...
class VorbisFile {
    ...
    @Throws(JOrbisException::class)
    constructor(`is`: InputStream, initial: ByteArray, ibytes: Int) : super() {
    ...
    @Throws(JOrbisException::class)
    fun open(`is`: InputStream, initial: ByteArray?, ibytes: Int): Int {
        return open_callbacks(`is`, initial, ibytes)
    }

    @Throws(JOrbisException::class)
    fun open_callbacks(`is`: InputStream, initial: ByteArray?, ibytes: Int//, callbacks callbacks
    ): Int {
        val ret: Int
        datasource = `is`
    ...
    fun bitrate(i: Int): Int {
        ...
        if (i < 0) {
            ...
            return Math.rint((bits / time_total(-1)).toDouble()).toInt()
        } else {
            if (seekable) {
                // return the actual bitrate
                return Math.rint(((offsets[i + 1] - dataoffsets[i]) * 8 / time_total(i)).toDouble()).toInt()
            } else {
    ...
    constructor(file: String) : InputStream() {
        ...
        @Synchronized override fun mark(m: Int) {}
    ...
    companion object {
        ...
        fun fseek(fis: InputStream, off: Long, whence: Int): Int {
            if (fis is SeekableInputStream) {
                val sis = fis
                try {
                    if (whence == SEEK_SET) {
                        sis.seek(off)
        ...
        fun ftell(fis: InputStream): Long {
            try {
                if (fis is SeekableInputStream) {
                    return fis.tell()
                }
