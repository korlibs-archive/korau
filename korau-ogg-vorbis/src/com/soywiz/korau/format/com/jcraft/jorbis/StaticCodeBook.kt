...
constructor() {
    ...
    private fun maptype1_quantvals(): Int {
        var vals = Math.floor(Math.pow(entries.toDouble(), 1.0 / dim)).toInt()
    ...
    companion object {
        ...
        fun float32_pack(`val`: Float): Long {
            ...
            exp = Math.floor(Math.log(`val`.toDouble()) / Math.log(2.0)).toInt()
            mant = Math.rint(Math.pow(`val`.toDouble(), (VQ_FMAN - 1 - exp).toDouble())).toInt()
        ...
        fun ldexp(foo: Float, e: Int): Float {
            return (foo * Math.pow(2.0, e.toDouble())).toFloat()
        }
    }
}
