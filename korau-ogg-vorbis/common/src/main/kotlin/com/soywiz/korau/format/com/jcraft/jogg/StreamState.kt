...
class StreamState {
    ...
    fun body_expand(needed: Int) {
        if (body_storage <= body_fill + needed) {
            ...
            body_data.copyRangeTo(0, foo, 0, body_data.size)
            body_data = foo
        }
    }
    ...
    fun lacing_expand(needed: Int) {
        if (lacing_storage <= lacing_fill + needed) {
            ...
            lacing_vals.copyRangeTo(0, foo, 0, lacing_vals.size)
            lacing_vals = foo

            val bar = LongArray(lacing_storage)
            granule_vals.copyRangeTo(0, bar, 0, granule_vals.size)
            granule_vals = bar
        }
    }
    ...
    fun pagein(og: Page): Int {
        ...
        run {
            ...
            if (br != 0) {
                body_fill -= br
                if (body_fill != 0) {
                    body_data.copyRangeTo(br, body_data, 0, body_fill)
                }
                body_returned = 0
            }
            ...
            if (lr != 0) {
                // segment table
                if (lacing_fill - lr != 0) {
                    lacing_vals.copyRangeTo(lr, lacing_vals, 0, lacing_fill - lr)
                    granule_vals.copyRangeTo(lr, granule_vals, 0, lacing_fill - lr)
                }
                lacing_fill -= lr
                lacing_packet -= lr
                lacing_returned = 0
        ...
        if (bodysize != 0) {
            ...
            body_base.copyRangeTo(body, body_data, body_fill, bodysize)
            body_fill += bodysize
        }
    ...
    fun flush(og: Page): Int {
        ...
        "OggS".toByteArray().copyRangeTo(0, header, 0, 4)
        header[4] = 0x00
        ...
        lacing_fill -= vals
        lacing_vals.copyRangeTo(vals, lacing_vals, 0, lacing_fill * 4)
        granule_vals.copyRangeTo(vals, granule_vals, 0, lacing_fill * 8)
        body_returned += bytes
