...
(private val common: Common) {
   ...
   */
    ...
    fun init_layer3(down_sample_sblimit: Int) {
        ...
        for (i in 0..8) COS9[i] = Math.cos(MPG123.M_PI / 18.0 * i.toDouble()).toFloat()
        for (i in 0..8) tfcos36[i] = (0.5 / Math.cos(MPG123.M_PI * (i * 2 + 1).toDouble() / 36.0)).toFloat()
        for (i in 0..2) tfcos12[i] = (0.5 / Math.cos(MPG123.M_PI * (i * 2 + 1).toDouble() / 12.0)).toFloat()
