...
public class Layer1 {
    ...
    private void I_step_one(final mpstr_tag mp, final int balloc[], final int scale_index[], final Frame fr) {
        int ba = 0;
        int sca = 0;
        ...
        if (fr.stereo == 2) {
            ...
            for (i = jsbound; i < MPG123.SBLIMIT; i++) balloc[ba++] = common.getbits(mp, 4);
            ...
            int i;
            for (i = 0; i < MPG123.SBLIMIT; i++) balloc[ba++] = common.getbits(mp, 4);
            ba = 0;
            for (i = 0; i < MPG123.SBLIMIT; i++) if ((balloc[ba++]) != 0) scale_index[sca++] = common.getbits(mp, 6);
        }
    }

    private void I_step_two(final mpstr_tag mp, final float fraction[][], final int balloc[], final int scale_index[], final Frame fr) {
        int i, n;
        ...
        if (fr.stereo == 2) {
            ...
            for (i = fr.down_sample_sblimit; i < 32; i++) fraction[0][i] = fraction[1][i] = 0.0f;
        } else {
            int f0 = 0;
            ba = 0;
    ...
    public int do_layer1(final mpstr_tag mp, final float[] pcm_sample, final ProcessedBytes pcm_point) {
        int clip = 0;
